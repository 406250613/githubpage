...
const APP_SHELL_URL = new URL(
  "./litter-box-tracker-pantry_cloud.html",
  self.location,
).toString();
...
self.addEventListener("install", (event) => {
  event.waitUntil(
    caches
      ...
      .then(() => self.skipWaiting()), // 确保缓存完成后再激活
  );
});
...
self.addEventListener("fetch", (event) => {
  ...
  // Network-First for HTML app shell — 保证能拿到最新版本，离线时回退缓存
  if (
...
self.addEventListener("push", (event) => {
  const PANTRY_ID = "3cd7c478-6fd3-4d4b-ae02-a36124e1e68a"; // 与应用主体保持一致
